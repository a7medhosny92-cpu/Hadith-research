...
- **★ STEP 5 DONE — كنية/«ابن» SHADOW fold wired into `collapse_duplicates` (this session).** The ~83 «كنية» are
  ...
  **★ STEP 7 (resolution-on-ingest) ASSESSED → REDUNDANT, DO NOT BUILD (2026-06-17).** Investigating the ingest path
  (`build_rijal.merge_source` + `dedup.collapse_duplicates`) end-to-end: the user's chosen **Option A (rebuild-time
  canonical, no persistent IDs)** IS `collapse_duplicates`, which runs at the END of `build_rijal` over EVERY record —
  including the coverage-ADDED ones (`fill_gaps=False` appends, then collapse folds via the kunya-shadow/prefix/deep
  passes). So all three step-7 jobs are already covered, each by its own path: (a) **no doublings** → collapse
  (removable ~1); (b) **enrich** (nasab/kunya/death/alias/opinions) → `_merge_into` whenever the coverage form is FULLER
  (added → collapsed → enriched); (c) **أقوال الأئمة** → `merge_appraisals` (separate name-match); (d) **network** →
  `build_graph` (separate). The ONE thing a resolve-on-ingest would add — enriching an EXISTING man on a coverage
  CONFIDENT match (today dropped, `merge_source` line 138) — is exactly the case that **MUST stay dropped**: الثقات/لسان
  are men OUTSIDE the Six Books, so a confident name-match to a تقريب man is almost always a NAMESAKE (different
  population), and enriching would graft another man's death/kunya (الإصابة has no death/kunya to give anyway). → the
