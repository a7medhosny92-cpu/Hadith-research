...
- **★ أقوال الأئمة — NAMED multi-critic verdicts (user: «riportare anche i loro nomi»).** The رجال books are not one
  ...
  **385 green**, node --check clean. **NEEDS A `build_rijal` (update.bat) with 2170/3722 on disk to populate** (then the
  ...
  - **★ (2026-06-16) +BOOK tag (user: «indicando anche il libro»).** Each appraisal now carries the **book that reports it**
    — `merge_appraisals(records, prose, book=)` tags every `{critic, verdict}` with `book` (تهذيب الكمال/الجرح والتعديل/
    الثقات/لسان الميزان, from the `_PROSE` map); `RijalEntry.appraisals` carries it through (raw dicts); the «راوٍ» card
    shows «ثقة — ابن معين · تهذيب الكمال» (`.r-book` span). So the researcher sees مَن قال، ماذا قال، **وفي أيّ كتاب**. +1
    test (436 green), node --check clean. Docs: التقنية RijalEntry {critic, verdict, **book**}. **NEEDS a `build_rijal`** to repopulate.
