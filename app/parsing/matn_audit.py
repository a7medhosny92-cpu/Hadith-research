...
_EDITORIAL = re.compile(
    r"هذا\s+حديث|هذا\s+اسناد|هذا\s+خبر|علي\s+شرط|اخرجه|اخرجاه|رواه|وفي\s+الباب|"
...
def flag_matn(matn: str, isnad: str, chapter: str = "") -> list[tuple[str, str]]:
    ...
    # I — a narration verb, or a leading «عن فلان», in the matn = isnad spilled past the boundary.
    if _CHAIN_VERB.search(mn) or _STARTS_AN.search(mn):
        out.append(("I", "في المتن أداةُ إسناد (حدثنا/أخبرنا أو «عن فلان» في أوله)"))

    # G — a grade / takhrīj / editorial note inside the matn.
