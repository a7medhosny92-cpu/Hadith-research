...
_COMPANION = re.compile(
    ...
    r"أحد العشرة|من العشرة|العشرة المبشرة|أسلم قديمًا|أسلم قديما|"
...
# and events: أسلم، ولد، تزوّج، ولي، ذكره، والدة…) keeps the name from swallowing the whole
# tarjama — e.g. «… أم سلمة أم المؤمنين [تزوجها النبي …]» or «عمر … العدوي [يقال له …]».
# NB «قيل/يقال» are NOT cut tokens: تقريب uses «ويقال ابن فلان» for an ALTERNATE nasab MID-name —
# «عمرو بن عبد الله بن عبيد ويقال ابن علي … الهمداني أبو إسحاق السبيعي» — so cutting there dropped the
# kunya + nisba (أبو إسحاق السبيعي ⇒ unreachable, hence «أبي إسحاق» fell to a صحابي homonym). They are
# instead handled by `_ALT_NASAB`/`_QIL_BARE` below (the alternate run is stripped, the name continues).
...
_DABT_FIXED = ("بفتح بضم بكسر بسكون بالفتح بالضم بالكسر بالسكون بالتشديد بالتخفيف بتشديد بتخفيف "
               "بالتصغير تصغير وفتح وضم وكسر وسكون وتشديد وتخفيف بفتحتين بضمتين بكسرتين بفتحات بضمات "
               "بكسرات مصغر مكبر مصغرا مكبرا مخففا منقوط منقوطة مقصور ممدود بعدها بعده أوله اوله "
               "أولها اولها آخره اخره وآخره واخره بينهما بينهم").split()
_DABT_ADJ = ("مفتوحة مضمومة مكسورة ساكنة مشددة مخففة ثقيلة خفيفة مهملة معجمة موحدة موحّدة "
             "موحدتين مثناة مثلثة تحتانية فوقانية مقصورة ممدودة مثقلة "
             "مفتوحتين مضمومتين مكسورتين ساكنتين مشددتين مهملتين معجمتين مثلثتين مثناتين "
             "تحتانيتين فوقانيتين معجمات مهملات").split()
...
for _w in _DABT_ADJ:
    _DABT |= {_w, "و" + _w, "ب" + _w, "ال" + _w, "وال" + _w, "بال" + _w, "وبال" + _w}
...
def _trim_name(text: str) -> str:
    ...
    padded = f" {text} "
    # first remove an interjected ALTERNATE nasab «(و)يقال ابن فلان» (and any bare leftover «(و)يقال»),
    # so the name isn't truncated there and runs on to the kunya/nisba (… أبو إسحاق السبيعي).
    padded = _QIL_BARE.sub(" ", _ALT_NASAB.sub(" ", padded))
    end = len(padded)
