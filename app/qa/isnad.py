...
_SKIP = {"قال", "قالا", "قالوا", "يعني", "قالت", "ح", "بهذا", "بهذه",
         # «قرأتُ على فلانٍ وأنا أسمع» — a سماع/قراءة حال, not a narrator: drop «(و)أنا (و)أسمع» so it
         # neither glues onto the man read-to nor (split_conarrators) becomes a bogus «أنا أسمع» node.
         "انا", "اسمع", "وانا", "واسمع"}
...
# A مرفوع SAYING attribution in the matn opener — «… قال: قال رسول الله ﷺ …» / «… سمعت النبيَّ ﷺ يقول …»
# — where the Prophet ﷺ carries his salutation but is in the report, NOT a chain node. A saying verb
# (قال/سمعت/يقول) DIRECTLY before «رسول الله ﷺ»/«النبي ﷺ» marks the report as reaching the Prophet ﷺ even
# when the chain ends at the terminal Companion's «قال» — but a mere mention/description «… قال كان النبيُّ
# ﷺ …» does NOT (no saying verb directly before him). (Without this a Companion who mis-resolves to a ثقة
# namesake — «ابن عمر» → نافع مولى ابن عمر — stays held «يُتوقَّف» on a صحيح chain, the terminal-صحابي
# promotion being gated on reaching the Prophet ﷺ.)
_PROPHET_ATTR = re.compile(r"(?:قال|قالت|يقول|سمعت|فقال)\s+(?:رسول\s+الله|(?:ال)?نب[يى])\s*(?:ﷺ|صلى)")
...
_WAW_STOP = {"وهو", "وهي", "وهم", "وهما", "وغير", "وغيره", "وغيرها", "وغيرهم", "واخر", "واخرون",
             ...
             "وهذا", "وكلاهما", "وكلهم", "وجميعا", "ومن", "وفي", "وقد", "وانا", "واسمع"}
...
def analyze_isnad(
    ...
    # marfūʿ iff the chain ends at the Prophet ﷺ node, OR the report is attributed to him with the
    # salutation in the matn opener («عن [صحابيّ] قال: قال رسول الله ﷺ …») — the ﷺ is reserved for him,
    # so the chain reaches him through the terminal Companion even though ﷺ isn't a chain node. This lets
    # the terminal-صحابي promotion run (else «ابن عمر» mis-resolved to نافع مولى ابن عمر holds a صحيح chain).
    reaches_prophet = bool(narrators) and is_prophet(narrators[-1].name)
    if not reaches_prophet and narrators and _PROPHET_ATTR.search(raw):
        reaches_prophet = True
